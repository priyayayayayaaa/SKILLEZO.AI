...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center p-8 text-center rounded-2xl bg-slate-900/40 border border-slate-800/80 backdrop-blur-md ${className}`}
    >
      ...
      {actionLabel && onAction && (
        <button
          onClick={onAction}
          className="px-4 py-2 rounded-xl bg-[#3D5AFE] hover:bg-[#3D5AFE]/90 text-white text-xs sm:text-sm font-medium transition-all shadow-md shadow-[#3D5AFE]/20"
        >
          {actionLabel}
        </button>
      )}
