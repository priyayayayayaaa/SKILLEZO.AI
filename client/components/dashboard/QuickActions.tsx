      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {mockQuickActions.map((action) => (
          <Link
            key={action.id}
            href={action.href}
            className="flex items-start gap-4 p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 hover:border-[#3D5AFE]/50 hover:bg-slate-800/40 transition-all duration-200 group"
          >
            <div className="p-3 rounded-xl bg-slate-800/80 border border-slate-700/50 group-hover:scale-105 transition-transform shrink-0">
              {actionIcons[action.iconName] || <CheckSquare className="w-5 h-5 text-[#3D5AFE]" />}
            </div>

            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-1 mb-1">
                <span className="text-sm font-bold text-slate-100 group-hover:text-[#3D5AFE] transition-colors truncate">
                  {action.label}
                </span>
                <ArrowUpRight className="w-4 h-4 text-slate-500 group-hover:text-slate-200 transition-colors shrink-0" />
              </div>
