...
const iconCategoryMap: Record<string, React.ReactNode> = {
  ...
  security: <ShieldAlert className="w-5 h-5 text-amber-400" />,
  system: <Info className="w-5 h-5 text-indigo-400" />
};
  ...
  return (
    <div
      className={`p-4 sm:p-5 rounded-2xl border transition-all duration-200 backdrop-blur-md flex items-start gap-4 ${
        !notification.read
          ? 'bg-slate-900/80 border-[#3D5AFE]/40 shadow-md shadow-[#3D5AFE]/5'
    ...
    >
      ...
      <div className="flex-1 min-w-0 space-y-1">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <h4 className="text-sm font-bold text-slate-100 truncate">{notification.title}</h4>
            {!notification.read && (
      ...
      <button
        ...
        className="p-1.5 rounded-xl text-slate-500 hover:text-slate-200 hover:bg-slate-800 transition-colors shrink-0"
