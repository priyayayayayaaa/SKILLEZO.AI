...
const activityIconMap: Record<string, React.ReactNode> = {
  ...
  User: <User className="w-4 h-4 text-emerald-400" />,
      ...
      <div className="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-800">
        {mockActivityTimeline.map((item) => (
          <div key={item.id} className="relative flex items-start gap-4 group">
            ...
            <div className="absolute -left-6 top-0.5 w-5 h-5 rounded-full bg-[#0B1130] border-2 border-slate-700 group-hover:border-[#3D5AFE] flex items-center justify-center transition-colors">
              <div className="w-1.5 h-1.5 rounded-full bg-[#3D5AFE]" />
            </div>

            <div className="flex-1 bg-slate-900/60 border border-slate-800/80 p-3.5 rounded-2xl group-hover:border-slate-700 transition-colors">
