...
const iconCategoryMap: Record<string, React.ReactNode> = {
  ...
  security: <ShieldAlert className="w-4 h-4 text-amber-400" />,
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 min-w-[16px] h-[16px] flex items-center justify-center rounded-full bg-[#00D9C0] text-[9px] font-bold text-[#0B1130] ring-2 ring-[#0B1130]">
            {unreadCount}
          </span>
        )}
      </button>
      ...
      {/* Dropdown Panel */}
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-2xl bg-[#0F172A]/95 border border-slate-800 shadow-2xl backdrop-blur-xl z-50 animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden">
          {/* Header */}
          <div className="p-4 border-b border-slate-800/80 flex items-center justify-between">
            ...
            {unreadCount > 0 && (
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#00D9C0]/15 text-[#00D9C0] border border-[#00D9C0]/30">
                {unreadCount} New
              </span>
            )}
          </div>
            ...
            {items.slice(0, 5).map((notification) => (
              <div
                ...
                className={`p-3.5 flex items-start gap-3 hover:bg-slate-800/40 transition-colors ${
                  !notification.read ? 'bg-slate-900/80' : 'opacity-75'
                }`}
              >
                ...
                <div className="flex-1 min-w-0 space-y-0.5">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs font-bold text-slate-100 truncate">{notification.title}</span>
                    {!notification.read && (
                ...
                >
                  <Check className={`w-3.5 h-3.5 ${notification.read ? 'text-emerald-400' : ''}`} />
                </button>
              </div>
            ))}
          </div>
          ...
          <div className="p-3 border-t border-slate-800/80">
            <Link
              ...
              className="flex items-center justify-center gap-2 w-full py-2 rounded-xl bg-slate-800/60 hover:bg-slate-800 text-xs font-semibold text-[#3D5AFE] hover:text-[#00D9C0] transition-colors"
            >
